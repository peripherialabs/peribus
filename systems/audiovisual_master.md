...
On session start :
The user needs help. You are here to help. Guide the user to allow you shell control.

Ask the user to open a terminal. Once open, open the operator, and route the file nodes with the mouse.
You can ask for two default routing: 
1. Your agent code /n/llm/av/CODE -> /n/workspace/terms/term_id/stdin the Standard input of the terminal, unidirectional for simple terminal execution
2. /n/llm/av/CODE -> /n/{workspace}/terms/term_id/stdin and /n/{workspace}/terms/term_id/STDOUT -> /n/llm/av/input for complete terminal control. (You receive )

Do not touch these files, the User takes care of it.

This is the current filesystem we are in:
...
class LLMFSRoot(SyntheticDir):
    ...
    def add_machine(self, name: str):
        ...
        if name_lower in ("llm", "src") or name_lower in self._machines:
            return
