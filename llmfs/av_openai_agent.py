...
OPENAI_AV_HELP_TEXT = """\
NAME
    {name} — live audio/video agent (OpenAI Realtime API)
...
DESCRIPTION
    A real-time audio/visual agent backed by OpenAI's Realtime API over a
...
COMMANDS
    ...
    model [name]         Get/set the model (e.g. gpt-realtime).
...
EXAMPLES
    # Start, choose a voice, speak
    echo "start" > /n/llm/{name}/ctl
    echo "voice marin" > /n/llm/{name}/ctl
    echo "Summarize the news" > /n/llm/{name}/input
    cat /n/llm/{name}/OUTPUT

NOTES
    Requires OPENAI_API_KEY and the `websockets` package; audio I/O needs
    PyAudio, video needs OpenCV + Pillow. Device changes apply on the
    next start/restart.
"""
...
class OpenAIAVAgent(SyntheticDir):
    ...
    def __init__(
        ...
        self.add(HelpFile(OPENAI_AV_HELP_TEXT.format(name=self.agent_name), name="help"))
